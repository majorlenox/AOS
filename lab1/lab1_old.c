...
int prog1(){
	...
	int fd = syscall(SYS_open, filename, O_RDWR, 0644);
	if (fd == -1){
		// 2 - ENOENT, 13 - EACCES
		printf("errno: %d\n", errno);
		printf("strerror: %s\n", strerror(errno));
	...
	syscall(SYS_write, fd, text_to_write, sizeof(text_to_write)-1);
...
int prog2(){
	...
	fd = syscall(SYS_open, filename, O_CREAT | O_WRONLY | O_TRUNC, mode); // you can't open created file without write permission
	if (fd == -1)
		perror("Can't create file");
	bytes = syscall(SYS_write, fd, text_to_write1, sizeof(text_to_write1)-1);
	bytes += syscall(SYS_write, fd, text_to_write2, sizeof(text_to_write2)-1);
	printf("%ld bytes were written\n", bytes);
	if (bytes <= 0) perror("Error");
	syscall(SYS_close, fd);

	fd = syscall(SYS_open, filename, O_RDONLY);
	bytes = syscall(SYS_read, fd, buf, 255);
	printf("%ld bytes were read\n", bytes);
	if (bytes <= 0) perror("Error");
	syscall(SYS_close, fd);

	fd = syscall(SYS_open, filename, O_RDWR);
	if (fd == -1){
...
int prog4(){
	...
	scanf("%4s", buf); // 0644
	mode = strtol(buf, NULL, 8);
	fd = syscall(SYS_open, filename, O_CREAT | O_RDWR | O_TRUNC, mode);
	if (fd == -1) {
		perror("Can't create file");
		return errno;
	}
	...
	while (option != 0){
		...
		switch(option){
			case 1: // write
				...
				scanf("%255[^\n]%n", buf, &r_count);
				getchar();
				bytes = syscall(SYS_write, fd, buf, r_count);
				printf("%ld bytes were written\n", bytes);
			break; 
			case 2: // read
			...
			case 3: // change seek
				...
				if ((option1 > 0) && (option1 < 4)){
					printf("Enter seek:\n");
					scanf("%ld", &pos);
					syscall(SYS_lseek, fd, pos, seeks[option1-1]);
					printf("Seek changed to %ld\n", syscall(SYS_lseek, fd, 0, SEEK_CUR));
				}
			break;
			default:	
			break;
...
int coping(){
	...
	while ((r_bytes = syscall(SYS_read, STDIN_FILENO, buf, sizeof(buf))) > 0) {
        	w_bytes = syscall(SYS_write, STDOUT_FILENO, buf, r_bytes);
        	if (w_bytes != r_bytes) {
            		perror("Can't read");
            		return -1;
        	}
...
int copy_file(){
	...
	scanf("%s", filename2);
	int src = -1, dst = -1;
	int saved_stdin = syscall(SYS_dup, STDIN_FILENO), saved_stdout = syscall(SYS_dup, STDOUT_FILENO);
	if (filename1[0] != '!'){
	      src = syscall(SYS_open, filename1, O_RDONLY);
	      if (src == -1){
		perror("Can't open file");
		syscall(SYS_close, saved_stdin);
                syscall(SYS_close, saved_stdout);
	        return 1;
	      }
	      syscall(SYS_close, STDIN_FILENO); // change stdin
	      syscall(SYS_dup2, src, STDIN_FILENO);
	}
	if (filename2[0] != '!'){
	      dst = syscall(SYS_open, filename2, O_WRONLY | O_CREAT | O_TRUNC, 0644);
	      if (dst == -1){
		perror("Can't open file");
		if (src != -1) {
                  syscall(SYS_close, STDIN_FILENO);
                  syscall(SYS_dup2, saved_stdin, STDIN_FILENO);
                }
                syscall(SYS_close, saved_stdin);
                syscall(SYS_close, saved_stdout);
	        return 1;
	      }
	      syscall(SYS_close, STDOUT_FILENO); // change stdout
	      syscall(SYS_dup2, dst, STDOUT_FILENO);
	}	
	coping();
        ...
        syscall(SYS_dup2, saved_stdout, STDOUT_FILENO);
    
        syscall(SYS_close, saved_stdin);
        syscall(SYS_close, saved_stdout);
        if (src != -1) syscall(SYS_close, src);
        if (dst != -1) syscall(SYS_close, dst);
	return 0;
}
...
int read_backwards(){
  ...
  off_t file_size = syscall(SYS_lseek, fd, 0, SEEK_END);
  if (file_size == -1) {
  ...
  for (off_t pos = file_size - 1; pos >= 0; pos--) {
        ...
        if (syscall(SYS_lseek, fd, pos, SEEK_SET) == -1) {
            perror("Can't seek in file");
            return 1;
        }
        
        if (syscall(SYS_read, fd, &ch, 1) != 1) {
